...
2. **Backward-compatible serialization.** New fields: default in `__init__`, written in `to_dict()`, read via `_input_dict.get("key", default)` in `from_dict()`, copied in `duplicate()`. Apply the **hbjson-serialization-contract** skill.
